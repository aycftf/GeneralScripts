...
doas auditctl -l | cut -d ' ' -f 6
echo -e "\n\n\n"
sleep 3

doas auditctl -s
sleep 2
echo -e "\n Live Log Command and Location: doas tail -f /var/log/audit/audit.log \n "
echo -e "\n Live Log Command and Location: doas tail -f /var/log/messages"
sleep 2
sudo rm loop.txt
...
if [ "$create" == "y" ]; then 
	...
	doas auditctl -w "$dir" -p wa -k "$name"
	sleep 1
	doas auditctl -l | grep "$name"
	echo "Imported keyword and rule name above here:"
	sleep 1
...
echo -e "Checking for any logged data... "

#Implement date feature later to check only from specific dates ... 

#Put all specific auditkeywords in a temp file
doas auditctl -l | cut -d ' ' -f 6 >> loop.txt
python3 audit.py
doas ausearch -m avc
doas ausearch -m ANOM_ADD_ACCT
doas ausearch -m ANOM_MOD_ACCT
doas ausearch -m ANOM_LOGIN_FAILURES
